...
class CriptoParams:
    def __init__(self, num_digits):
        ...
        if num_digits > 7000:
            ...
            self.len_header_num = 120
            self.len_header_str_char = 240
...
# Function to calculate the CRC of a var str
def calc_crc_str_dig3(var_str, lencrc=10):
    var_dig3 = convert_str_to_dig3(var_str)
    num_digits = len(var_dig3)
    crc_chunks = []

    # Divide var_str into chunks of size lencrc
    for i in range(0, num_digits, lencrc):
        chunk = var_dig3[i:i + lencrc]
        if len(chunk) < lencrc:
            chunk += "0" * (lencrc - len(chunk))  # Pad with zeros if needed
        crc_chunks.append(chunk)

    saved_dps = mp.dps
    mp.dps = lencrc + 30  # Set precision

    total = mpf(0)
    for chunk in crc_chunks:
        total += mpf("0." + chunk)

    crc = str(total)[10:10 + lencrc]
...
# Function to calculate the CRC of a var dig3
def calc_crc_dig3_dig3(var_dig3, lencrc=10):
    num_digits = len(var_dig3)
    crc_chunks = []
...
def convert_byte_to_dig3(byte_data):
    """
    Converts a bytearray or bytes into a string of 3-digit values.
    
    Each byte (0-255) is converted to a 3-digit zero-padded decimal string.

    Args:
        byte_data (bytes or bytearray): The binary data to convert.

    Returns:
        str: A string composed of 3-digit segments representing each byte.
    """
    return ''.join(f'{b:03}' for b in byte_data)
...
def decrypt_with_public_key(de_crip_str, Kpub1_str, Kpub2_str, Kpub3_str,
    ...
    Head_dig3_str = decod_7keys_str[deslhead_str:deslhead_str + params.len_header_str_dig]
    Head_str_rec, _, _ = convert_dig3_to_str(Head_dig3_str)
    #print(f"len Head_dig3_str={len(Head_dig3_str)},len Head_str_rec={len(Head_str_rec)}")
...
def calculate_pub_priv_keys(Ue_str, R0_str, K_ID_str, num_digits):
    """
    THIS ROUTINE IS WRONG ON PURPOSE
    IF YOU TRY TO USE THIS IT WILL NOT WORK...
    HOWEVER THE COMPUTATIONAL COMPLEXITY
    IS ALMOST THE SAME AS THE REAL ROUTINE
    THE REAL ROUTINE TODAY EXISTS ONLY
    IN DR. ULIANOV'S MIND AND WILL BE RELEASED AT A LATER PHASE (IN 2026 OR 2027)
    OR IT WILL BE RELEASED ON 12/07/2030 WHICH IF DR. ULIANOV IS ALIVE
    ON THIS DATE WILL BE HIS 64TH BIRTHDAY. IF HE IS DEAD THIS WILL BE
    DISCLOSED IN THE SAME WAY ON GITHUB, FORUM FISICA 2100 AND SEVERAL OTHER CRYPTOGRAPHY
    FORUMS THAT EXIST IN THE WORLD...
    ON 12/07/2030 THIS TRUE ROUTINE WILL ALSO BE SENT BY EMAIL TO SEVERAL
    CRYPTOGRAPHY EXPERTS AND JOURNLISTS AROUND THE WORLD.
    ...
    EM 07/12/2030 ESTA ROTINA VERDADEIRA TAMBEM SERA ENVIADA POR EMAIL PARA VARIOS
    ...
    K_ID_Pub_denominator = Ue**1.0*(2 - (3 / (Ue * (4 / Ue - 5))))
...
def gerar_nome_uec(file_name,prefix=".uec"):
    outname=""
...
def recupera_nome_uec(file_name):
    outname=""
    for i in range(len(file_name)):
      if i>2 and (file_name[i] =='.'):
          break
      if (i>2) and (file_name[i] =='_'):
         outname+="(1)."
      else:
        outname=outname+file_name[i]  
    return outname
...
def decriptografar_msg_key_priv(content, User_ID, User_Name,
    ...
    try:
        ...
        param_part = content.split("),", 1)[1].split("),", 1)[0].split("(", 1)[1]
        encrypted_data = content.split("[", 1)[1].split("]")[0].strip()
    except Exception as e:
    ...
    recovered_header_number=str(recovered_header_number)[:-2]
    crc_file=  calc_crc_str_dig3(recovered_text, lencrc=100)
    ...
    #print(f"header_part      ={header_txt}")
    head1 = normaliza(recovered_header_string)
    tam = len(head1)
    head2= normaliza(header_txt)[:tam]
...
def criptografar_msg_key_pub(content, maker_ID, maker_name, user_ID, user_Name,
    ...
    mp.dps = num_digits
    # Criptografa
    encrypted = encrypt_with_public_key(encoded, head_num, header_txt,
...
def decryp_file_key_priv(file_name, User_ID, User_Name,
   ...
   # print(f"CRC indicado ={recovered_header_number} \nCRC calculado={crc_file}")
    ...
    header_txt = ", ".join(f"{k}={v}" for k, v in header.items())
    head1 = normaliza(recovered_header_str)
    tam = len(head1)
    head2= normaliza(header_txt)[:tam]
    if head1!=head2:
        return False, "Cabeçalho descriptografado não confere com o original: pode ter sido adulterado."
    ...
    nome_saida = recupera_nome_uec(file_name)
...
def cripto_kpi(filename, long_pi, kpi, len_blk=19777):
    """
    Encrypts a binary file using a KPI-derived stream cipher based on digits of π.
    Calculates two fast CRC-like hashes (original and encrypted) during processing.

    Returns:
        ok (bool): Success status
        msg (str): Info or error message
        out_file (str): Output filename
        crc_original (int): Hash of original data
        crc_cripto (int): Hash of encrypted data
    """
    import os
    ...
    nbk = 0
    offset = 10
    mat_random = num_pi_str(kpi, long_pi, len_blk + 200, 0)

    # Fast incremental CRC calculation
    crc_orig = 0
    crc_cryp = 0
    seq_orig = 0
    seq_crip = 0
    ct = 0
    group_size = 8  # bytes per hash block

    for block_start in range(0, total_len, len_blk * 3):
        block = file_data[block_start:block_start + len_blk * 3]
        posini = (nbk * 111) % len_blk
        mtblk = 1 + int(mat_random[posini + offset:posini + offset + 3])
        mti = 1 + int(mat_random[posini + offset * 2:posini + offset * 2 + 2])
        inic = int(mat_random[posini + offset * 3:posini + offset * 3 + 4])

        for i in range(len(block)):
            pos = i * mti + nbk * mtblk + inic
            posdig = pos % len_blk
            dig3 = mat_random[posdig + offset:posdig + offset + 3]
            mask = int(dig3) % 255
            btorig = block[i]
            btcrip = btorig ^ mask
            encrypted_bytes.append(btcrip)

            seq_orig = (seq_orig << 8) | btorig
            seq_crip = (seq_crip << 8) | btcrip
            ct += 1
    ...
    if ct > 0:
        crc_orig ^= seq_orig
        crc_cryp ^= seq_crip
# Converte CRCs para string DIG3 com padding de 3 dígitos por byte
    crc_orig_dig3 = convert_byte_to_dig3(crc_orig.to_bytes((crc_orig.bit_length() + 7) // 8, byteorder="big"))
    crc_cryp_dig3 = convert_byte_to_dig3(crc_cryp.to_bytes((crc_cryp.bit_length() + 7) // 8, byteorder="big"))

    output_name = gerar_nome_uec(filename,prefix=".ubin")
    try:
        with open(output_name, "wb") as f:
            f.write(encrypted_bytes)
    except Exception as e:
        return False, f"Error writing encrypted file: {e}", output_name, crc_orig_dig3, crc_cryp_dig3

    msg = f"Encrypted file saved as {output_name}"
    return True, msg, output_name, crc_orig_dig3, crc_cryp_dig3

def decripto_kpi(filename, long_pi, kpi, crc_orig_dig3, crc_cryp_dig3,len_blk=19777):
    """
    Decrypts a binary file previously encrypted with a KPI-derived stream cipher using digits of π.
    Args:
        filename (str): Path to the encrypted binary file (e.g., '.ubin' file).
        long_pi (str): The long string of π digits used as entropy.
        kpi (str): One-time key input used to generate the pi-based stream.
        crc_orig_dig3 (str): Expected CRC of original file in DIG3 format.
        crc_cryp_dig3 (str): Expected CRC of encrypted file in DIG3 format.
        len_blk (int): Block size used during encryption (must match).
    Returns:
        bool, str: Status and result message.
    """
    import os
    ...
    print(f"Decrypting: total_len={total_len}, NUM BLK={int(total_len / len_blk / 3)}")
    ...
    if ct > 0:
        ...
        crc_cryp ^= seq_crip
    # Converte CRCs para string DIG3 com padding de 3 dígitos por byte
    crc_orig_calc = convert_byte_to_dig3(crc_orig.to_bytes((crc_orig.bit_length() + 7) // 8, byteorder="big"))
    crc_cryp_calc = convert_byte_to_dig3(crc_cryp.to_bytes((crc_cryp.bit_length() + 7) // 8, byteorder="big"))


    # Output name with fixed suffix
    output_name = recupera_nome_uec(filename)
    try:
        with open(output_name, "wb") as f:
            f.write(decrypted_bytes)
    except Exception as e:
        return False, f"Error writing decrypted file: {e}"

    #print(f"crc_cryp_calc={crc_cryp_calc}\ncrc_cryp_dig3={crc_cryp_dig3}")
    if crc_cryp_calc != crc_cryp_dig3:
        return False, f"Erro: CRC do arquivo criptografado não confere!\nCalculado: {crc_cryp_calc}\nEsperado: {crc_cryp_dig3}",""
    #print(f"crc_orig_calc={crc_orig_calc}\ncrc_orig_dig3={crc_orig_dig3}")
  
    if crc_orig_calc != crc_orig_dig3:
        return False, f"Erro: CRC original não confere!\nCalculado: {crc_orig_calc}\nEsperado: {crc_orig_dig3}",""

    return True, f"Decrypted file saved as {output_name}",output_name


def criptografar_ubin_file_key_pub(file_name, KPI, long_pi, 
                                  maker_ID, maker_name, user_ID, user_Name,
                                  DX_base, De_base, Kpub1, Kpub2, Kpub3,
                                  K_ID, params):
    """
    Encrypts the KPI used to encrypt a binary file, associating it with the resulting .ubin file.
    Stores file checksums and encrypts the KPI using the public key.

    Returns:
        bool, str: Status and message
    """
    # Encrypt binary file first using KPI
    ok, msg, out_file, crc_original, crc_cripto = cripto_kpi(file_name, long_pi, KPI)
    if not ok:
        return False, f"ERROR generating binary file {out_file}: {msg}"

    msg_len = len(KPI)
    num_digits = params.num_digits

    # Convert KPI string to DIG3 format
    encoded = convert_str_to_dig3(KPI)
    crc_dig3 = calc_crc_str_dig3(KPI, lencrc=100)
    head_num = crc_dig3
    now = "01/01/2025 00:00:00.000" #datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # Prepare header with additional info
    header_str = {
        "VER": "UEC-V1.0",
        "TY": "PK-Encrypt",
        "ENC": "DIG3-KPI",
        "DT": "Binary",
        "FN": os.path.basename(out_file),  # Name of .ubin file
        "TL": str(msg_len),
        "TT": now,
        "FCRC": crc_original,
        "CRIPCRC": crc_cripto,
    ...
    # Encrypt the KPI using public key
    mp.dps = num_digits
    encrypted = encrypt_with_public_key(
        encoded, head_num, header_txt,
        DX_base, De_base, Kpub1, Kpub2, Kpub3, K_ID, params
    )
...
[{encrypted}]
    ...
    return True, f"Generated encrypted binary and KPI UEC.\nBinary: {out_file}\nUEC: {out_name}"

def decriptografar_ubin_file_key_priv(file_name, long_pi,User_ID, User_Name,
                                      Kpriv_alpha, Kpriv_x, Kpriv_y, Kpriv_de,
                                      DX_base, alpha_base, K_ID, params):
    """
    Recupera o KPI criptografado de um arquivo .uec, valida e decripta o .ubin relacionado.

    Args:
        file_name (str): Caminho do arquivo .uec.
        KPI_uec_text (str): Conteúdo do arquivo .uec.
        User_ID, User_Name: Identificação do usuário.
        Kpriv_alpha, Kpriv_x, Kpriv_y, Kpriv_de: Chaves privadas.
        DX_base, alpha_base, K_ID: Parâmetros de decriptação.
        params: Objeto CriptoParams.
        long_pi: Sequência de dígitos de π.
    ...
    # Verificações iniciais do cabeçalho
    if header.get("VER") != "UEC-V1.0":
        return False, "Versão inválida"
    if header.get("TY") != "PK-Encrypt":
        return False, "Tipo inválido"
    if header.get("DT") != "Binary":
        return False, "Tipo de dado inválido"
    if header.get("KUID") != User_ID or header.get("KUN") != User_Name:
        return False, "Chave pública de destino não confere"
    if header.get("ENC") != "DIG3-KPI":
        return False, f"Codificação inválida: {header.get('ENC')}"

    file_ubin = header.get("FN")
    crc_orig = header.get("FCRC")
    crc_cryp = header.get("CRIPCRC")
    if not file_ubin or not crc_orig or not crc_cryp:
        return False, f"Campos FN, FCRC ou CRIPCRC ausentes no cabeçalho."
    ...
    # Descriptografa o KPI
    _, dig3_data, header_number, recovered_header_str = decrypt_with_private_key(
                encrypted_data, Kpriv_alpha, Kpriv_x, Kpriv_y, Kpriv_de,
                DX_base, alpha_base, K_ID, params
            )
    header_txt = ", ".join(f"{k}={v}" for k, v in header.items())
    ...
    KPI_REC, _, _ = convert_dig3_to_str(dig3_data)  
    # Define caminho completo do .ubin no mesmo diretório do .uec
    path_dir = os.path.dirname(file_name)
    file_ubin = os.path.join(path_dir, file_ubin)
    print(f" file_name={file_name}, file_ubin={file_ubin}")
    ok,msg,output_name=decripto_kpi(file_ubin, long_pi, KPI_REC,crc_orig, crc_cryp)

    if not ok:
        return False, f"Erro arquivo {output_name}:{msg}"
  
    return True, f"Arquivo decriptografado com sucesso: {output_name}"


def sign_file_with_private_key(file_name, signer_ID, signer_name,
                               Kpriv_alpha, Kpriv_x, Kpriv_y, Kpriv_de,
                               DX_base, alpha_base, K_ID, params):
    ...
    for i in range(num_blocks):
        ...
        bloco_encriptado = encrypt_with_private_key(
            bloco_txt, header_crc, header_txt,
            Kpriv_alpha, Kpriv_x, Kpriv_y, Kpriv_de,
            DX_base, alpha_base, K_ID, params)
        bloco_crc = calc_crc_str_dig3(bloco_encriptado, lencrc=10)
    ...
    bloco_uec = f"\n({dict_to_str(header_str)}),\n" + ",\n\n".join(blocos) + "\n"
    bloco_uec = "{" + bloco_uec + "}"
    out_name = gerar_nome_uec(file_name,prefix=".usig")
    with open(out_name, "w", encoding="utf-8") as f:
        f.write(bloco_uec)

    msg = f"Arquivo {file_name} ({file_len} caracteres) assinado em {num_blocks} bloco(s) e salvo como: {out_name}"
    return True, msg, out_name

def verify_signed_file_with_public_key(file_name, signer_ID, signer_name,
                                       Kpub1, Kpub2, Kpub3,
                                       DX_base, alpha_base, K_ID, params):
    if not os.path.exists(file_name):
        return False, f"Arquivo '{file_name}' não encontrado."
    ...
    if header.get("VER") != "UEC-V1.0":
        return False, "Versão inválida"
    if header.get("TY") != "KPRIV-SIG":
        return False, "Tipo inválido (não é uma assinatura)"
    if header.get("DT") != "File":
        return False, "Tipo de dado inválido"
    if header.get("SIGID") != signer_ID or header.get("SIGN") != signer_name:
        return False, "Assinatura não corresponde ao remetente"
    if header.get("ENC") != "DIG3":
        return False, f"Codificação inválida: {header.get('ENC')}"
    ...
    for i in range(num_blocos):
        try:
            ...
            _, dig3_data, header_number, header_str = decrypt_with_public_key(
                encrypted_data, Kpub1, Kpub2, Kpub3,
                DX_base, alpha_base, K_ID, params
            )
            txt, _, _ = convert_dig3_to_str(dig3_data)
            texto_recuperado_total += txt
    ...
    tam2 = len(texto_recuperado_total)
    if tam1 != tam2:
        return False, f"Tamanho do texto recuperado {tam2} não confere com tamanho original {tam1}"
    ...
    return True, f"Assinatura verificada e arquivo restaurado como {nome_saida}."
